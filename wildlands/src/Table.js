...
function getData() {
  return fetch(`${process.env.PUBLIC_URL}/olof.json`)
    .then(res => res.json())
    .then(data => data)
}
...
const setupRows = async () => {
  ...
  const mang = data.profiles.map(d => {
    ...
    .map(d => {
      ...
      const kills = gs(d, 'KilledEnemiesCount')
      const timeplayed =  gs(d, 'SoloPlaytime')/3600 + gs(d, 'CoopPlaytime')/ 3600
      const kph = kills / timeplayed
      const cqc = gs(d, 'KillWithCQCCount')
      const noalert = gs(d, 'CompletedMissionWithNoAlertCount')
      const rev = gs(d, 'RevivedTeammateCount')
      const stealth = gs(d, 'StealthKillPercentage')
  ...
  const rows = mang.map(m => { 
    const { name, kph, kills, timeplayed, noalert, stealth } = m
    return {name, kph, kills, timeplayed, noalert, stealth }
  })
